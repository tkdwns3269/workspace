...
ORDER BY DEPT_CODE;     --4 ORDER BY 무조건 마지막 실행 -> 정렬은 마지막에 하는게 아니면 의미 X

-- 각 직급별 총사원수, 보너스를 받는 사람수, 급여합, 평균급여, 최저급여 (정렬= 직급 오름차순)
SELECT JOB_CODE AS "직급",
       COUNT(*) AS "사원수", 
       COUNT(BONUS) AS "보너스받는 사람 수",
       SUM(SALARY) AS "급여합",
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') AS 성별, COUNT(*) AS 인원
FROM EMPLOYEE
...
--각 부서별 평균 급여가 300만원 부서들만 조회(부서코드 평균급여)
...
--직급별 직급코드, 총 급여합(단, 직급별 총 급여합이 1000만원 이상인 직급만 조회
...
/*
    ...
    - UNION : OR | 합집합(두 쿼리문 수행한 결과값을 더한다_
...
--1. 컬럼의 개수가 동일해야한다.
--2. 컬럼자리마다 동인한 타입으로 기술해야한다.
--3. 정렬하고싶다면 ORDER BY는 마지막에 기술한다.

--UNION ALL : 여러개의 쿼리결과를 무조건 다 더한다(중복 상관X)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
