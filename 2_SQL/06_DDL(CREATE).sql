/*
    *데이터 정의 언어
    오라클에서 제공하는 객체를 새로 만들고(CREATE), 구조를 변경하고(ALTER), 구조체를 삭제(DELETE)하는 언어
    즉, 실제 데이터값이 아닌 규칙 자체를 정의하는 언어
    
    오라클에서 객체(구조) : 테이블, 뷰, 시퀸스
                                    인덱스, 패키지, 트리거
                                    프로시져, 함수, 동의어, 사용자
    <CREATE>
    객체를 새로 생성하는 구문
...
/*
        1.테이블 생성
        -테이블이란: 행과 열로 구성되는 가장 기본적인 테이터베이스 객체
        ...
        *자료형
        -문자 (CHAR(바이트크기) | VARCHAR2(바이트크기)) -> 반드시 크기 지정을 해줘야 함
                ...
                -VARCHAR2 : 최대 4000 바이트까지 지정 가능/ 가변길이(몇글자의 데이터가 들어올지 모를경우)
...
INSERT INTO MEMBER VALUES(1, 'USER1', 'PASS1', '홍길동', '남', '010-1111-2222', 'AAAA@NAVER.COM', '24/08/12');
...
/*
        *NOT NULL
        해당 컬럼에 반드시 값이 존재해야한 말 경우( 즉, 절대 NULL이 들어오면 안돼는 경우)
        삽입/ 수정시 NULL값을 허용하지 않도록 제한
        
        제약조건을 부여하는 방식은 크게 2가지가 있다. (컬럼레벨방식, 테이블레벨방식)
        NOT NULL제약 조건은 무조건 컬럼레벨방식으로만 가능하다.
...
INSERT INTO MEM_NOTNULL 
VALUES(3, NULL, 'PASS3', '홍길삼', '남', NULL, NULL, NULL, NULL);
-- 아이디가 중복되었음에도 불구하고 잘 추가된다.
...
CREATE TABLE MEM_UNIQUE (
        ...
        MEM_ID VARCHAR2(20) CONSTRAINT MEMID_NT NOT NULL UNIQUE, --컬럼레벨방식 
...
INSERT INTO MEM_UNIQUE
VALUES(2, 'USER1', 'PASS2', '홍길순', '여', '010-1111-3333', 'BBBB@NAVER.COM', '24/01/02');


--UNIQUE 제약조건에 위배되었으므로 INSERT 실패
--> 오류구준
--> 쉽게 파악하기가 어렵다.
--> 제약조건 부여시 제약조건 명을 지정할 수 있다 (지정하지 않으면 시스템에서 자동으로 부여함)
...
/*
        *CHECK (조건식)
        해당 컬럼에 들어올 수있는 값에 대한 조건을 제시해둘 수 있다.
...
--> GENDER 컬럼에는 CHECK제약조건을 만족하는 값을 넣어야 한다.
...
/*
        ...
        EX) 회원번호, 학번, 군번 ,부서코드, 직급코드 , 주민번호, 운송장 번호, 예약번호 등 ...
...
CREATE TABLE MEM_PRI (
        MEM_NO NUMBER CONSTRAINT MEMNO_PR PRIMARY KEY,
        MEM_ID VARCHAR2(20) NOT NULL UNIQUE, --컬럼레벨방식 
        MEM_PWD VARCHAR2(20) NOT NULL,
        MEM_NAME VARCHAR2(20) NOT NULL,
        GENDER CHAR(3) CHECK (GENDER IN('남', '여')),  --남, 여
...
--> 기본키에 중복값을 담으려고 할 때 (NOT NULL제약조건 위배)
...
INSERT INTO MEM_PRI2
VALUES (2, 'USER3', 'PASS3', '홍길사', '나', NULL, NULL, NULL);
...
INSERT INTO MEM
VALUES (2, 'USER2', 'PASS2', '홍길순', '여', NULL, NULL, 10);

INSERT INTO MEM
VALUES (3, 'USER3', 'PASS3', '홍길삼', '여', NULL, NULL, 20);

INSERT INTO MEM
VALUES (4, 'USER4', 'PASS4', '홍길사', '여', NULL, NULL, 30);

INSERT INTO MEM
VALUES (5, 'USER5', 'PASS5', '박지원', '여', NULL, NULL, 40);

--유효하지 않은 회원등급번호가 정상적으로 INSERT 되고 있다.
...
/*
        ...
        ->참조할 컬럼명 생략시 참조할 테이블의 PRIMARY KEY로 지정된 컬럼에 매칭된다.
...
INSERT INTO MEM
VALUES (4, 'USER4', 'PASS4', '김수민', '여', NULL, NULL, 30);
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
-- MEM테이블에서 30이라는 값을 사용하고 있지 않기 때문에 삭제가 가능하다.
...
/*
        자식테이블 생성시 외래키 제약조건 부여할 때 삭제옵션을 부여할 수 있다.
        *삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식 테이블의 값을 어떻게 할 것인가?
        
        -ON DELETE RESRICTED (기본값) : 삭제제한옵션, 자식데이터로부터 쓰이는 부모데이터는 삭제가 아예 안됨
        -ON DELETE SET NULL : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식데이터의 값을 NULL로 변경
        -ON DELETE CASCADE : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식데이터도 같이 전부 삭제
...
INSERT INTO MEM VALUES (1, 'USER1', 'PASS1', '홍길동', '남', NULL, NULL, NULL);

INSERT INTO MEM VALUES (3, 'USER3', 'PASS3', '홍길삼', '여', NULL, NULL, 20);

INSERT INTO MEM VALUES (4, 'USER4', 'PASS4', '김수민', '여', NULL, NULL, 30);
...
/*
        ...
        컬럼을 선정하지 않고 INSERT시 NULL이 아닌 기본값을 INSERT 하고자한다.
        이때 세팅해 둘 수 있는 값
...
--INTSERT INTO 테이블명 VALUES (값1, 값2, . . . )
...
--> 단, 해당컬럼에 DAFAULT값이 부여되어 있을 경우 NULL이 아닌 DEFAULT값이 들어감
...
/*
        ...
        -FOREIGN KEY : ALTER TABLE 테이블명 ADD CONSTRAINT FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명( 컬럼명);
        -UNIQUE : ALTER TABLE 테이블명 ADD UNIQUE KEY(컬럼명);
        -CHEKCH : ALTER TABLE 테이블명 ADD CHECK(컬럼에 대한 조건식);
